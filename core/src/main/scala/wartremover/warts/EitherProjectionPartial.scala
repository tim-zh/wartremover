      ...
      override def traverse(tree: Tree): Unit = {
        tree match {
          ...
          case Select(left, GetName) if left.tpe.baseType(leftProjectionSymbol) != NoType =>
            u.error(tree, "LeftProjection#get is disabled - use LeftProjection#toOption instead")
            super.traverse(tree)
          case Select(left, GetName) if left.tpe.baseType(rightProjectionSymbol) != NoType =>
            u.error(tree, "RightProjection#get is disabled - use RightProjection#toOption instead")
            super.traverse(tree)
